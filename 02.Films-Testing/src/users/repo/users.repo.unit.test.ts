...
describe('Given a instance of UsersRepo class', () => {
  ...
  describe('When method register is called', () => {
    // Espiamos un método de este componente. Nos devuelve si ha sido llamado o no
    vi.spyOn(AuthService, 'hash').mockResolvedValue('password-hasheada');
  ...
  describe('When method login is called with valid data', () => {
    ...
    test('Then it return the login result if user data are INVALID', async () => {
      // Arrange
      (prismaMock.user.findUniqueOrThrow as Mock).mockResolvedValueOnce({
        id: 1,
        email: 'test@example.com',
  ...
  describe('When method getUserById is called', () => {
    ...
    describe('And the user with the given id NOT exists', () => {
      test('Then it throw an error', async () => {
        // Arrange
        (prismaMock.user.findUniqueOrThrow as Mock).mockRejectedValueOnce(
          new Error('User not found'),
        );
        // Act & Assert
        await expect(repo.getUserById(999)).rejects.toThrow('User not found');
      });
    });
  });
  ...
  describe('When method updateUser is called', () => {
    ...
    describe('And the user with the given id NOT exists', () => {
      test('Then it throw an error', async () => {
        ...
        // Act & Assert
        await expect(repo.updateUser(999, {} as UserUpdateDTO)).rejects.toThrow(
          'User not found',
        );
      });
    });
  ...
  describe('When method updateUserProfile is called', () => {
    describe('And the userProfile with the given id exists', () => {
      test('Then it return the updated user', async () => {
        // Act
  ...
  describe('When method deleteUser is called', () => {
    ...
    describe('And the user with the given id NOT exists', () => {
      test('Then it throw an error', async () => {
        ...
        // Act & Assert
        await expect(repo.deleteUser(999)).rejects.toThrow('User not found');
      });
    });
  });
});
